...
interface IStorage {
  ...
  function getPollsWithDeadlineAfterToday();
  function getPollsWithDeadlineBeforeToday();
...
class Storage implements IStorage {
  ...
  public function getPollsWithDeadlineAfterToday() {
    $today = date('Y-m-d');
    return array_filter($this->contents, function ($item) use ($today) {
        return $item['deadline'] >= $today;
    });
  }

  public function getPollsWithDeadlineBeforeToday() {
    $today = date('Y-m-d');
    return array_filter($this->contents, function ($item) use ($today) {
        return $item['deadline'] < $today;
    });
